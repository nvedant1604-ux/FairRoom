  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-lg border border-blue-100 bg-white shadow-soft">
        <div className="border-b border-blue-100 bg-gradient-to-r from-blue-50 via-white to-green-50 p-5 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide text-blue-700 shadow-sm">
                ...
                Redevelopment Allocation Control Center
              </div>
              <h2 className="mt-3 text-2xl font-bold text-navy sm:text-3xl">
                {stats.society?.redevelopment_project_name ?? "Redevelopment Room Allocation"}
              </h2>
              <p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-600">
                <span className="inline-flex items-center gap-1">
                  <Home aria-hidden="true" className="h-4 w-4 text-blue-700" />
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center lg:justify-end">
              ...
              {!isAdmin ? (
                <div className="flex items-center gap-3 text-xs font-semibold text-slate-700 sm:basis-full lg:justify-end">
                  ...
                  <button className="text-blue-700 underline" onClick={onAdminLogin} type="button">Admin Login</button>
                </div>
              ) : null}
            </div>
          </div>
      ...
      {activeCompetitive ? <section className="rounded-lg border border-purple-200 bg-purple-50 p-5"><p className="font-bold text-purple-900">Ready for Competitive Lottery</p><p className="mt-1 text-sm text-purple-800">{activeCompetitive.draw_name} · all selected eligible residents may participate.</p><div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">{[["Eligible Residents",activeCompetitive.total_eligible??0],["Available Rooms",activeCompetitive.total_available_rooms],["Potential Winners",potentialWinners],["Potential Non-Winners",potentialNonWinners]].map(([label,value])=><div className="rounded bg-white p-3" key={String(label)}><p className="text-xs font-semibold uppercase text-slate-500">{label}</p><p className="text-2xl font-bold text-navy">{value}</p></div>)}</div></section> : null}
