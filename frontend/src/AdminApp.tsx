...
const pagePaths: Record<PageKey, string> = {
  ...
  residentDashboard: "/resident/dashboard", residentProfile: "/resident/profile", residentEligibility: "/resident/eligibility", residentLottery: "/resident/lottery", residentAllocation: "/resident/allocation", residentHistorySelf: "/resident/history"
};
const pathPages = Object.fromEntries(Object.entries(pagePaths).map(([key, path]) => [path, key])) as Record<string, PageKey>;
const protectedPages = new Set<PageKey>(["residents", "rooms", "lottery", "results", "audit", "report", "residentSearch", "residentHistory", "drawHistory", "drawCycleSetup", "eligibility"]);

function pageFromLocation(): PageKey {
  if (window.location.pathname === "/admin/login") return "admin";
  if (window.location.pathname === "/admin/dashboard") return "dashboard";
...
export default function App() {
  ...
  const [loginNotice, setLoginNotice] = useState<string | null>(() => {
    const notice = window.sessionStorage.getItem("fairroom_admin_session_notice");
    window.sessionStorage.removeItem("fairroom_admin_session_notice");
    return notice;
  });
  ...
  useEffect(() => {
    if (sessionStatus === "authenticated") void refreshAll();
    else { setDashboard(null); setResidents([]); setRooms([]); setAllocations([]); setAudits([]); setReport(null); setLoading(false); }
  ...
  function login(token: string) {
    ...
    window.dispatchEvent(new Event("fairroom-admin-login"));
  ...
  async function logout() {
    ...
    window.dispatchEvent(new Event("fairroom-admin-logout"));
  ...
  const page = (() => {
    switch (activePage) {
      ...
      case "residentSearch": return protectedContent(<ResidentSearch />);
